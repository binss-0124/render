    ...
    forceDeath(attacker) { //**수정
      if (this.player && !this.isDead) {
        ...
        if (this.gameUI && typeof this.gameUI.addDeath === 'function') {
          this.gameUI.addDeath(this.player.name); //**수정
        }
        if (attacker && this.gameUI && typeof this.gameUI.addKill === 'function') {
            this.gameUI.addKill(attacker.name);
        }
